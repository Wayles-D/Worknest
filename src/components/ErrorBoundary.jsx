...
export default function ErrorBoundary() {
  ...
  const redirect = () => {
    if (error?.status === 404 || msgs.includes(details)) {
  ...
  return (
    <div className="container mx-auto p-4 flex flex-col items-center justify-center min-h-screen gap-2">
      ...
      <button
        onClick={redirect}
        type="button"
        className="my-4 p-3 cursor-pointer rounded-lg btn bg-[#AA3E1F] hover:bg-[#FFA366] text-white"
      >
        {msgs.includes(details) ? "Refresh" : error?.status === 404 ? "Go Back" : "Go Back"}
      </button>
    </div>
  );
}
